...
# Conditional import for Model Armor (optional GCP dependency)
try:
    ...
    # Model Armor not available - LlmAsAJudge will still work without it
    HAS_MODEL_ARMOR = False
    SanitizeModelResponseResponse = None
    SanitizeUserPromptResponse = None
    CsamFilterResult = None
    MaliciousUriFilterResult = None
    RaiFilterResult = None
    PiAndJailbreakFilterResult = None
    SdpFilterResult = None
    FilterMatchState = None
...
def parse_model_armor_response(
    response: Any,  # SanitizeModelResponseResponse | SanitizeUserPromptResponse when Model Armor is available
) -> list[tuple[str, Any]] | None:
    """Analyzes the Model Armor response and returns a list of detected filters."""
    if not HAS_MODEL_ARMOR:
        raise ImportError("google-cloud-modelarmor is not installed. This function requires Model Armor.")
...
def parse_csam_filter(csam_result: Any) -> list[str]:
...
def parse_rai_filter(rai_result: Any) -> list[str]:
...
def parse_sdp_filter(sdp_result: Any) -> list[str]:
